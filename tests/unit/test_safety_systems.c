 ...
 *
 * @note Now uses HAL Abstraction Layer for clean testing
 */
...
#include "test_api_stubs.h" // TODO: Remove when APIs are properly implemented
...
// Forward declaration of mock state structure
typedef struct MockHAL_State_t MockHAL_State_t;

// Mock control functions (defined in hal_abstraction_mock.c)
extern void HAL_Abstraction_Mock_Reset(void);
extern MockHAL_State_t *HAL_Abstraction_Mock_GetState(void);
...
void setUp(void) {
    // Reset HAL abstraction mock state before each test
    HAL_Abstraction_Mock_Reset();

    // Initialize HAL abstraction layer
...
void test_emergency_stop_activation(void) {
    // Simulate emergency stop button press using HAL abstraction
    HAL_Abstraction_Mock_SetGPIOState(HAL_GPIO_PORT_C, GPIO_PIN_13,
...
void test_watchdog_refresh_normal_operation(void) {
    uint32_t initial_count =
        HAL_Abstraction_Mock_GetState()->watchdog_refresh_count;
...
void test_watchdog_timeout_detection(void) {
    ...
    HAL_Abstraction_Mock_AdvanceTick(5000 + 100); // 5s timeout + margin
