...
void HAL_GPIO_WritePin(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, GPIO_PinState PinState) {
    ...
    if (mock_hal_state.gpio_state_count < MAX_GPIO_STATES) {
        MockGPIO_State_t* state = &mock_hal_state.gpio_states[mock_hal_state.gpio_state_count];
        state->port = GPIOx;
        state->pin = GPIO_Pin;
...
GPIO_PinState HAL_GPIO_ReadPin(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
    ...
    if (GPIOx == GPIOA && GPIO_Pin == GPIO_PIN_0) {
        return mock_hal_state.emergency_stop_state ? GPIO_PIN_SET : GPIO_PIN_RESET;
    }
