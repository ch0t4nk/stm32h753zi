...
#include "stm32h7xx.h"
#include <math.h>
/* Prefer oscillator values from SSOT to avoid hardcoded literals across the
 * codebase. SSOT provides SSOT_HSE_VALUE, SSOT_CSI_VALUE, SSOT_HSI_VALUE.
 */
#include "config/clock_config.h" /* Named reset/PLL values + exception registry */
#include "config/project_constants.h"

/* Allow SSOT to override default oscillator values used by system startup */
#ifndef HSE_VALUE
#define HSE_VALUE SSOT_HSE_VALUE
#endif /* HSE_VALUE */

#ifndef CSI_VALUE
#define CSI_VALUE SSOT_CSI_VALUE
#endif /* CSI_VALUE */

#ifndef HSI_VALUE
#define HSI_VALUE SSOT_HSI_VALUE
#endif /* HSI_VALUE */
...
/* FPU settings ------------------------------------------------------------*/
    ...
    /* Reset CFGR register (named reset for traceability)
     * HARDCODED EXCEPTION: See clock_config.h
     * REASON: Vendor-defined reset; changing may alter boot clock behavior.
     */
    RCC->CFGR = RCC_CFGR_RESET_VALUE;

    /* Reset HSEON, HSECSSON, CSION, HSI48ON, CSIKERON, PLL1ON, PLL2ON and
     * PLL3ON bits
     * HARDCODED EXCEPTION: See clock_config.h (RCC_CR_RESET_CLEAR_MASK)
     * REASON: Mask derived from vendor reset sequence; localized to startup
     *         to avoid unintended clock side-effects.
     */
    RCC->CR &= RCC_CR_RESET_CLEAR_MASK;
...
#endif
    /* Reset PLLCKSELR register (use named SSOT value)
     * HARDCODED EXCEPTION: See clock_config.h
     * REASON: Vendor reset table value - must match reference for safe boot.
     */
    RCC->PLLCKSELR = RCC_PLLCKSELR_RESET;

    /* Reset PLLCFGR register (use named SSOT value)
     * HARDCODED EXCEPTION: See clock_config.h
     * REASON: Vendor reset table value - central but intentionally precise.
     */
    RCC->PLLCFGR = RCC_PLLCFGR_RESET;
    /* Reset PLL1DIVR & PLL1FRACR (named SSOT values)
     * HARDCODED EXCEPTION: See clock_config.h
     * REASON: Reset values are vendor-defined; keep symbolic for audits.
     */
    RCC->PLL1DIVR = RCC_PLL1DIVR_RESET;
    /* Reset PLL1FRACR register */
    RCC->PLL1FRACR = RCC_PLL1FRACR_RESET;

    /* Reset PLL2DIVR/FRACR registers (named SSOT values)
     * HARDCODED EXCEPTION: See clock_config.h
     * REASON: Vendor reset values; centralized for traceability.
     */
    RCC->PLL2DIVR = RCC_PLL2DIVR_RESET;
    RCC->PLL2FRACR = RCC_PLL2FRACR_RESET;
    /* Reset PLL3DIVR/FRACR registers (named SSOT values)
     * HARDCODED EXCEPTION: See clock_config.h
     * REASON: Vendor reset values; centralized for traceability.
     */
    RCC->PLL3DIVR = RCC_PLL3DIVR_RESET;
    RCC->PLL3FRACR = RCC_PLL3FRACR_RESET;

    /* Reset HSEBYP bit (preserved literal from vendor code)
     * TAG: HARD_CODED - Reason: clearing bypass bit is explicit and matches
     *       vendor step; kept local because it's a single-bit action tied to
     *       board-specific HSE configuration and test coverage is hardware.
     */
    RCC->CR &=
        0xFFFBFFFFU; /* HSEBYP clear - will add to exceptions if needed */
            ...
            case RCC_PLLCKSELR_PLLSRC_HSI: /* HSI used as PLL clock source */
                ...
                pllvco =
                    ...
                    ((float_t)(uint32_t)(RCC->PLL1DIVR & RCC_PLL1DIVR_N1) +
                     (fracn1 / SSOT_PLL_FRACN_DIVISOR) + (float_t)1);
