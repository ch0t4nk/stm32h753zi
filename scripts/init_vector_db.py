...
class STM32VectorDatabase:
    ...
    def __init__(self, db_path: str = "./vector_db"):
        ...
        # Initialize client early to avoid None type issues
        try:
            self.db_path.mkdir(parents=True, exist_ok=True)
            self.client = chromadb.PersistentClient(path=str(self.db_path))
            print(f"✅ ChromaDB client initialized at {self.db_path}")
        except Exception as e:
            print(f"⚠️  ChromaDB client initialization deferred: {e}")
            # Client will be initialized later in initialize_database()
    ...
    def initialize_database(self) -> bool:
        ...
        try:
            ...
            # Create persistent client using new ChromaDB API (if not already initialized)
