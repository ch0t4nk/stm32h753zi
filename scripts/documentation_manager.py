...
def create_stm32h7_index():
    ...
    if not STM32H7_DOC_ROOT.exists():
        print(f"❌ STM32H7 documentation directory not found: {STM32H7_DOC_ROOT}")
        return False
    ...
    for md_file in STM32H7_DOC_ROOT.rglob('*.md'):
        ...
        for keyword in keywords:
            peripheral_match = re.match(r'^(GPIO|UART|USART|SPI|I2C|ADC|DAC|TIM|DMA|DMAMUX|RCC|PWR|EXTI|NVIC|CRC|RNG|HASH|CRYP|LTDC|DSI|DCMI|ETH|USB|CAN|FDCAN|SAI|DFSDM|MDIOS|SDMMC|FMC|QUADSPI|OCTOSPI|HSEM|WWDG|IWDG|RTC|TAMP|LPTIM|LPUART|SWPMI|OPAMP|COMP|VREFBUF|CORDIC|FMAC)', keyword)
            if peripheral_match:
                index_data['peripheral_index'][peripheral_match.group(1)].append(relative_path)
            ...
            # Index concepts
            concept_keywords = ['interrupt', 'IRQ', 'callback', 'DMA', 'initialization', 'configuration', 'polling', 'blocking', 'buffer', 'FIFO', 'register', 'clock', 'timeout', 'error']
            if any(concept in keyword.lower() for concept in concept_keywords):
                index_data['concept_index'][keyword.lower()].append(relative_path)
    ...
    print(f"✅ STM32H7 index created: {total_files} files, {round(total_size / (1024 * 1024), 1)}MB")
    return True
...
def create_l6470_index():
    ...
    for md_file in L6470_DOC_ROOT.rglob('*.md'):
        ...
        # L6470 registers
        registers = re.findall(r'\b(ABS_POS|EL_POS|MARK|SPEED|ACC|DEC|MAX_SPEED|MIN_SPEED|KVAL_HOLD|KVAL_RUN|KVAL_ACC|KVAL_DEC|INT_SPEED|ST_SLP|FN_SLP_ACC|FN_SLP_DEC|K_THERM|ADC_OUT|OCD_TH|STALL_TH|FS_SPD|STEP_MODE|ALARM_EN|CONFIG|STATUS)\b', content)
        ...
        # L6470 commands
        commands = re.findall(r'\b(NOP|SetParam|GetParam|Run|StepClock|Move|GoTo|GoTo_DIR|GoUntil|ReleaseSW|GoHome|GoMark|ResetPos|ResetDevice|SoftStop|HardStop|SoftHiZ|HardHiZ|GetStatus)\b', content)
        for cmd in commands:
    ...
    print(f"✅ L6470 index created: {total_files} files, {round(total_size / (1024 * 1024), 1)}MB")
    return True
...
def extract_keywords_from_file(file_path):
    ...
    peripherals = re.findall(r'\b(GPIO|UART|USART|SPI|I2C|ADC|DAC|TIM|DMA|DMAMUX|RCC|PWR|EXTI|NVIC|CRC|RNG|HASH|CRYP|LTDC|DSI|DCMI|ETH|USB|CAN|FDCAN|SAI|DFSDM|MDIOS|SDMMC|FMC|QUADSPI|OCTOSPI|HSEM|WWDG|IWDG|RTC|TAMP|LPTIM|LPUART|SWPMI|OPAMP|COMP|VREFBUF|CORDIC|FMAC)\b', content)
    keywords.update(peripherals)
    
    # Extract structure names
    structs = re.findall(r'\b([A-Z][A-Za-z0-9_]*_(?:InitTypeDef|HandleTypeDef|TypeDef))\b', content)
...
def display_results(results, query):
    ...
    if stm32h7_results:
        ...
        for result in stm32h7_results:
            print(f"   • {result['file']} ({result['size_kb']}KB) - {result['type']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Documentation Search and Index Management Tool')
    
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # Index management commands
    index_parser = subparsers.add_parser('index', help='Manage documentation indexes')
    index_parser.add_argument('action', choices=['create', 'recreate'], help='Index action')
    index_parser.add_argument('--scope', choices=['all', 'STM32H7', 'L6470'], default='all', help='Index scope')
    
    # Search commands
    search_parser = subparsers.add_parser('search', help='Search documentation')
    search_parser.add_argument('type', choices=['peripheral', 'function', 'register', 'command', 'concept'], help='Search type')
    search_parser.add_argument('query', help='Search query')
    search_parser.add_argument('--scope', choices=['all', 'STM32H7', 'L6470'], default='all', help='Search scope')
