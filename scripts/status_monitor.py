...
class StatusMonitor:
    ...
    def _check_automation_active(self) -> bool:
        ...
        return all(
            hook.exists() and hook.stat().st_mode & 0o111 for hook in hooks
        )
    ...
    def format_status_bar(self, status: Dict) -> str:
        ...
        automation_icon = (
            "$(pulse)" if status["automation_active"] else "$(circle-slash)"
        )
    ...
    def watch_mode(self, interval: int = 5) -> None:
        ...
        print(
            f"📡 Starting STATUS monitor (interval: {interval}s)", flush=True
        )
        print(
            "   Watching for changes in build/, .git/, and STATUS.md",
            flush=True,
        )
        ...
        try:
            while True:
                ...
                print(
                    f"[{timestamp}] Build: {build_status} | Git: {git_status} | Auto: {automation}",
                    flush=True,
                )

                if status["flash_usage"] != "unknown":
                    print(
                        f"           Flash: {status['flash_usage']} | RAM: {status['ram_usage']}",
                        flush=True,
                    )
