...
def extract_links(file_path):
    ...
    link_patterns = [
        r'\[([^\]]*)\]\(([^)]+)\)',  # [text](url) - but exclude empty text with code-like URLs
        r'\[([^\]]*)\]:\s*([^\s]+)',  # [text]: url
    ]
            ...
            if len(match) == 2:
                ...
                if (not text.strip() and 
                    (url.startswith('<') or url.startswith('(') or 
                     'VERSION' in url or 'GPIO' in url or 'HAL_' in url or
...
def validate_workspace_links(workspace_root, exclude_dirs=None):
    ...
    for md_file in md_files:
        ...
        for text, url in links:
            ...
            # Validate internal links
                ...
                is_valid, status = check_internal_link(url, md_file, workspace_path)
                if not is_valid:
                    broken_links += 1
                    file_issues.append({
                        'type': 'broken_link',
        ...
        if file_issues:
            ...
            for issue in file_issues:
                if issue['type'] == 'html_link':
                    print(f"  🔗 HTML: [{issue['text']}]({issue['url']}) → Suggest: {issue['suggestion']}")
                else:
                    print(f"  ❌ BROKEN: [{issue['text']}]({issue['url']}) - {issue['status']}")
    ...
    print(f"Files with issues: {len([f for f in md_files if extract_links(f)])}")
