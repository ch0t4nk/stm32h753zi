...
DOC_INDEX_STM32H7_COPILOT_JSON = WORKSPACE_ROOT / "docs" / "indexes" / "STM32H7_COPILOT_INDEX.json"
...
def load_stm32h7_index():
    ...
    try:
        ...
        print(f"❌ STM32H7 index not found at SSOT path: {DOC_INDEX_STM32H7_FULL}")
        return None
...
def format_results(results, search_type, query, doc_type):
    ...
    if not results:
        print(f"❌ {search_type.title()} '{query}' not found in {doc_type} documentation")
        return
    
    print(f"✅ Found {len(results)} {doc_type} {search_type} results for '{query}':")
    print("=" * 60)
...
def main():
    ...
    if search_type == "peripheral":
        ...
        if scope == "all" and not any([
            ...
            print(f"❌ Function '{query}' not found in either documentation set")
