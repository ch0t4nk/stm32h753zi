...
class STM32Validator:
    ...
    def run_command(self, cmd, description=""):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.errors.append(f"❌ {description}: {cmd} (Exit code: {result.returncode})")
    ...
    def check_toolchain(self):
        ...
        # Check ARM GCC
        success, output = self.run_command("arm-none-eabi-gcc --version", "ARM GCC Compiler")
        if success:
            version = output.split('\n')[0]
            print(f"   Found: {version}")

        # Check GDB
        success, output = self.run_command("gdb-multiarch --version", "GDB Multiarch Debugger")
        if success:
            version = output.split('\n')[0]
            print(f"   Found: {version}")

        # Check OpenOCD
        success, output = self.run_command("openocd --version 2>&1 | head -1", "OpenOCD")
        if success:
            ...
            with open(extensions_file) as f:
                ...
                for ext in required_extensions:
                    if ext in recommended:
                        self.successes.append(f"✅ Extension recommended: {ext}")
                        print(f"   ✅ {ext}")
                    else:
                        self.warnings.append(f"⚠️ Extension not in recommendations: {ext}")
                        print(f"   ⚠️ {ext}")
        else:
    ...
    def check_reference_assets(self):
        ...
        for asset_path, description in assets.items():
            ...
            if full_path.exists():
                ...
                self.warnings.append(f"⚠️ {description}: {asset_path} not found")
            ...
            with open(settings_file) as f:
                ...
                for setting, description in required_settings.items():
                    if setting in settings:
                        ...
                        self.warnings.append(f"⚠️ {description}: {setting} not configured")
                        print(f"   ⚠️ {setting}")
