...
typedef struct {
  ...
  uint32_t last_sample_timestamp_us; ///< Last sample timestamp (microseconds)

  // AS5600 encoder state
  float last_position_degrees; ///< Previous position for velocity calculation
  float last_velocity_dps; ///< Previous velocity for acceleration calculation
...
optimization_telemetry_collect_sample(uint8_t motor_id,
  ...
  packet->sample_sequence_id = context->performance.total_samples_collected + 1;
  ...
  packet->cpu_utilization_percent = context->performance.cpu_overhead_percent;
  ...
  // Check safety bounds
  result =
      telemetry_check_safety_bounds(context, packet, &packet->safety_bounds_ok);
  if (result != SYSTEM_OK) {
  ...
  telemetry_update_performance_metrics(context, packet->control_loop_time_us);
...
SystemError_t optimization_telemetry_get_performance_metrics(
  ...
  memcpy(metrics, &context->performance, sizeof(TelemetryPerformanceMetrics_t));
...
SystemError_t optimization_telemetry_calculate_efficiency(
  ...
  if (electrical_power_w > 0.1f) { // Avoid division by zero
    *efficiency_percent = (mechanical_power_w / electrical_power_w) * 100.0f;
...
optimization_telemetry_export_json(const CharacterizationDataSet_t *dataset,
  ...
  for (uint32_t i = 0; i < max_samples && written < (int)buffer_size - 500;
    ...
    int sample_written = snprintf(
        ...
        "        \"safety_bounds_ok\": %s\n"
        "      }%s\n",
        (unsigned long)sample->timestamp_us, (double)sample->position_degrees,
        (double)sample->velocity_dps, (double)sample->motor_current_a,
        (double)sample->power_consumption_w, (double)sample->position_error,
        sample->data_quality_score, sample->safety_bounds_ok ? "true" : "false",
        (i < max_samples - 1) ? "," : "");
...
static SystemError_t telemetry_read_as5600_burst(uint8_t motor_id,
  ...
  result = telemetry_calculate_derivatives(
      context, *position_degrees, dt_seconds, velocity_dps, acceleration_dps2);
...
static SystemError_t telemetry_read_l6470_status_fast(
  ...
  *overcurrent_detected = (status_register & STATUS_OCD_MSK) != 0;
  ...
  if (result != SYSTEM_OK) {
    ...
    *motor_current_a = (float)kval_run * 0.001f; // KVAL to current conversion
  }
...
telemetry_calculate_checksum(const CharacterizationDataSet_t *dataset) {
  ...
  if (dataset->sample_count > 0) {
    ...
    if (dataset->sample_count > 1) {
      ...
      checksum ^=
          (uint32_t)(dataset->samples[last_idx].position_degrees * 1000.0f);
