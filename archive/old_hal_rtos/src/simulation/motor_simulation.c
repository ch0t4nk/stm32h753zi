...
static bool l6470_sim_validate_register_write(uint8_t reg_addr, uint32_t value);
...
bool motor_simulation_is_active(void) {
  return g_simulation_initialized && (g_simulation.mode != SIM_MODE_DISABLED);
}
  ...
  case SIM_MOTOR_CONSTANT_SPEED:
    ...
    if (motor->target_position != motor->current_position) {
      float distance_remaining =
          fabsf((float)(motor->target_position - motor->current_position));
      float decel_distance = (motor->current_speed * motor->current_speed) /
