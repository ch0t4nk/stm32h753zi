...
void MotorControlTask(void *argument) {
  ...
  for (;;) {
    ...
    while (xQueueReceive(motor_command_queue, &received_command, 0) == pdTRUE) {
    ...
    if (xSemaphoreTake(motor_state_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
      ...
      result = ERROR_BUSY;
      printf("MotorControlTask: Failed to acquire motor state mutex\r\n");
    }

    // Handle any errors from motor control
    if (result != SYSTEM_OK) {
      SafetyEvent_t safety_event = {.event_type = SAFETY_EVENT_MOTOR_ERROR,
                                    .error_code = result,
...
void SafetyMonitorTask(void *argument) {
  ...
  for (;;) {
    ...
    while (xQueueReceive(safety_event_queue, &received_event, 0) == pdTRUE) {
      ...
      if (received_event.event_type == SAFETY_EVENT_EMERGENCY_STOP) {
        ...
        if (motor_control_task_handle != NULL) {
          ...
          printf("SafetyMonitorTask: Motor control task suspended\r\n");
        }
      }
    }
...
// TODO: Integrate CAN communication when available
    ...
    if (can_protocol_receive(&received_message) == SYSTEM_OK) {
      ...
      if (can_protocol_parse_motor_command(&received_message, &motor_command) ==
          SYSTEM_OK) {
...
// TODO: Integrate UART communication when available
    ...
    if (xQueueReceive(uart_message_queue, &received_message, 0) == pdTRUE) {
    ...
    if ((uart_task_cycles % 20) == 0) { // Every 20 cycles = 1 second at 20Hz
...
void TelemetryTask(void *argument) {
  ...
  for (;;) {
    ...
    if (motor_control_task_handle != NULL) {
      motor_stack_free = uxTaskGetStackHighWaterMark(motor_control_task_handle);
    }
    ...
    // available
    TelemetryData_t telemetry_data = {.uptime_ms = uptime,
...
SystemError_t rtos_tasks_init(void) {
  ...
  telemetry_queue = xQueueCreate(TELEMETRY_QUEUE_SIZE, sizeof(TelemetryData_t));
  safety_event_queue =
  ...
  watchdog_timer =
      xTimerCreate("WatchdogTimer", pdMS_TO_TICKS(WATCHDOG_REFRESH_PERIOD_MS),
                   pdTRUE, // Auto-reload
...
SystemError_t rtos_tasks_start(void) {
  ...
  result = xTaskCreate(
      SafetyMonitorTask, "SafetyMonitor", SAFETY_MONITOR_TASK_STACK_SIZE, NULL,
  ...
  result = xTaskCreate(CANCommTask, "CANComm", CAN_COMM_TASK_STACK_SIZE, NULL,
                       CAN_COMM_TASK_PRIORITY, &can_comm_task_handle);

  if (result != pdPASS) {
    printf("RTOS Tasks: ERROR - Failed to create CAN Communication Task\r\n");
    return ERROR_MEMORY_ALLOCATION;
  }

  // Create UART Communication Task (Priority 1)
  result = xTaskCreate(UARTCommTask, "UARTComm", UART_COMM_TASK_STACK_SIZE,
                       NULL, UART_COMM_TASK_PRIORITY, &uart_comm_task_handle);
  ...
  result = xTaskCreate(TelemetryTask, "Telemetry", TELEMETRY_TASK_STACK_SIZE,
...
SystemError_t rtos_wait_system_startup(uint32_t timeout_ms) {
  ...
  EventBits_t result =
      xEventGroupWaitBits(system_event_group, required_events, pdFALSE, pdTRUE,
...
SystemError_t rtos_wait_motion_complete(uint32_t motors, uint32_t timeout_ms) {
  ...
  EventBits_t result =
      xEventGroupWaitBits(motion_event_group, required_events, pdTRUE, pdTRUE,
                          pdMS_TO_TICKS(timeout_ms));
...
static SystemError_t init_memory_pools(void) {
  ...
  const osMemoryPoolAttr_t small_pool_attr = {.name = "SmallPool",
                                              .attr_bits = 0U,
                                              .cb_mem = NULL,
                                              .cb_size = 0U,
  ...
  const osMemoryPoolAttr_t medium_pool_attr = {.name = "MediumPool",
                                               .attr_bits = 0U,
                                               .cb_mem = NULL,
                                               .cb_size = 0U,
                                               .mp_mem = medium_pool_storage,
                                               .mp_size =
                                                   sizeof(medium_pool_storage)};

  const osMemoryPoolAttr_t large_pool_attr = {.name = "LargePool",
                                              .attr_bits = 0U,
                                              .cb_mem = NULL,
                                              .cb_size = 0U,
                                              .mp_mem = large_pool_storage,
                                              .mp_size =
                                                  sizeof(large_pool_storage)};
...
SystemError_t rtos_get_memory_stats(MemoryPoolStats_t *stats) {
  ...
  stats->peak_usage = MEMORY_POOL_SMALL_BLOCK_COUNT - stats->available_blocks;
  stats->allocation_failures = 0; // TODO: Implement allocation failure tracking
