 ...
 *          This file provides system initialization template function is case
 *of an application using a dual core STM32H7 device where : Cortex-M4 boot is
 *enabled at the FLASH option bytes Cortex-M7 boot is disabled at the FLASH
 *option bytes Cortex-M7 boot can be enabled by the the Cortex-M4 (when needed)
