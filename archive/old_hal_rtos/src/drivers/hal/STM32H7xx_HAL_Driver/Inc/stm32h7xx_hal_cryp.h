...
typedef struct {
  uint32_t DataType; /*!< no swap(32-bit data), halfword swap(16-bit data), byte
                      swap(8-bit data) or bit swap(1-bit data).this parameter
                      can be a value of @ref CRYP_Data_Type */
  uint32_t KeySize;  /*!< Used only in AES mode : 128, 192 or 256 bit key length
                        in  CRYP1.    This parameter can be a value of @ref
                        CRYP_Key_Size */
  ...
  uint32_t KeyIVConfigSkip; /*!< CRYP peripheral Key and IV configuration skip,
                               to configure Key and Initialization Vector only
                               once and to skip configuration for consecutive
                               processing. This parameter can be a value of @ref
...
#define CRYP_DATAWIDTHUNIT_BYTE                                                \
  0x00000001U /*!< Size unit is byte, but all input will be loaded in HW CRYPT \
                 IP by block of 4 words */
