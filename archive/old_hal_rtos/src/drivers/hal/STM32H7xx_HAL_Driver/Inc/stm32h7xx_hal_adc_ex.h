...
typedef struct {
   ...
   * to 1023 for ADC1/2 and value of @ref ADC_HAL_EC_OVS_RATIO for ADC3*/
#else
...
typedef struct {
  ...
  uint32_t
      InjectedOffset; /*!< Defines the offset to be subtracted from the raw
                         converted data. Offset value must be a positive number.
                           Maximum value depends on ADC resolution and
                         oversampling ratio (in case of oversampling used). This
                         parameter must be a number between Min_Data = 0x0000
                         and Max_Data = 0x3FFFC00 (corresponding to resolution
                         16 bit and oversampling ratio 1024). Note: This
                         parameter must be modified when no conversion is on
                         going on both regular and injected groups (ADC
  ...
  uint32_t InjectedOffsetRightShift; /*!< Specifies whether the 1 bit
                                        Right-shift feature is used or not. This
                                        parameter is applied only for 16-bit or
                                        8-bit resolution. This parameter can be
  ...
  FunctionalState
      InjectedOffsetSignedSaturation; /*!< Specifies whether the Signed
                              saturation feature is used or not. This parameter
                              is applied only for 16-bit or 8-bit resolution.
...
typedef struct {
  ...
  uint32_t TwoSamplingDelay; /*!< Configures the Delay between 2 sampling
                                ...
                                cycles for 14 bits from 1 to 8 clock cycles for
                                12 bits from 1 to 6 clock cycles for 10 bits
...
#define ADC_EXTERNALTRIGINJEC_LPTIM1_OUT                                       \
  (LL_ADC_INJ_TRIG_EXT_LPTIM1_OUT) /*!< ADC group injected conversion trigger  \
                                      from external peripheral: LPTIM1 OUT     \
                                      event. Trigger edge set to rising edge   \
                                      (default setting). */
#define ADC_EXTERNALTRIGINJEC_LPTIM2_OUT                                       \
  (LL_ADC_INJ_TRIG_EXT_LPTIM2_OUT) /*!< ADC group injected conversion trigger  \
                                      from external peripheral: LPTIM2 OUT     \
                                      event. Trigger edge set to rising edge   \
                                      (default setting). */
#define ADC_EXTERNALTRIGINJEC_LPTIM3_OUT                                       \
  (LL_ADC_INJ_TRIG_EXT_LPTIM3_OUT) /*!< ADC group injected conversion trigger  \
 ...
 * @{
 */
#define ADC3_OFFSET_SIGN_NEGATIVE                                              \
  (0x00000000UL) /*!< Offset sign negative, offset is subtracted */
#define ADC3_OFFSET_SIGN_POSITIVE                                              \
  (ADC3_OFR1_OFFSETPOS) /*!< Offset sign positive, offset is added  */
/**
 * @}
 */
#endif
...
#define ADC_TWOSAMPLINGDELAY_1CYCLE                                            \
  (LL_ADC_MULTI_TWOSMP_DELAY_1CYCLE_5) /*!< ADC multimode delay between two    \
...
#define ADC_DFSDM_MODE_ENABLE                                                  \
  (LL_ADC_REG_DFSDM_TRANSFER_ENABLE) /*!< ADC conversion data are transferred  \
                                        to DFSDM for post processing. The ADC  \
                                        conversion data format must be 16-bit  \
                                        signed and right aligned, refer to     \
                                        reference manual. DFSDM transfer       \
                                        cannot be used if DMA transfer is      \
                                        enabled. */
/**
 * @}
 */
 ...
 * @retval None
 */
#define ADC3_CFGR_DMACONTREQ(__DMACONTREQ_MODE__)                              \
  ((__DMACONTREQ_MODE__) << ADC3_CFGR_DMACFG_Pos)
#endif
/**
 ...
 * to 0 If resolution 16 bits, no shift. If resolution 14 bits, shift of 2 ranks
 * on the left. If resolution 12 bits, shift of 4 ranks on the left. If
...
#define ADC3_OFFSET_SHIFT_RESOLUTION(__HANDLE__, __OFFSET__)                   \
  ((__OFFSET__) << ((((__HANDLE__)->Instance->CFGR & ADC3_CFGR_RES) >> 3UL) *  \
                    2UL))
...
#define ADC_AWD1THRESHOLD_SHIFT_RESOLUTION(__HANDLE__, __THRESHOLD__)          \
  (                                                                            \
      ((__HANDLE__)->Instance == ADC3)                                         \
          ? ((__THRESHOLD__)                                                   \
             << (((((__HANDLE__)->Instance->CFGR) & ADC3_CFGR_RES) >> 3UL) *   \
                 2UL))                                                         \
      : ((((__HANDLE__)->Instance->CFGR) & ADC_CFGR_RES_2) == 0UL)             \
          ? ((__THRESHOLD__)                                                   \
             << (((((__HANDLE__)->Instance->CFGR) & ADC_CFGR_RES) >> 2UL) *    \
                 2UL))                                                         \
          : ((__THRESHOLD__) << (((((__HANDLE__)->Instance->CFGR) &            \
                                   (ADC_CFGR_RES & 0xFFFFFFF3UL)) >>           \
                                  2UL) *                                       \
                                 2UL)))
#else
 ...
 * (__CALIBRATION_FACTOR__ is too large)
 */
#define IS_ADC_CALFACT_ADC3(__CALIBRATION_FACTOR__)                            \
...
#define IS_ADC3_DIFF_CHANNEL(__CHANNEL__)                                      \
  (((__CHANNEL__) == ADC_CHANNEL_1) || ((__CHANNEL__) == ADC_CHANNEL_2) ||     \
   ((__CHANNEL__) == ADC_CHANNEL_3) || ((__CHANNEL__) == ADC_CHANNEL_4) ||     \
   ((__CHANNEL__) == ADC_CHANNEL_5) || ((__CHANNEL__) == ADC_CHANNEL_10) ||    \
   ((__CHANNEL__) == ADC_CHANNEL_11) || ((__CHANNEL__) == ADC_CHANNEL_13) ||   \
   ((__CHANNEL__) == ADC_CHANNEL_14) || ((__CHANNEL__) == ADC_CHANNEL_15))
 ...
 * @param __CHANNEL__: programmed ADC channel.
 * @retval return the negative input channels corresponding to the selected
 * channel.
 */
#define ADC_CHANNEL_DIFF_NEG_INPUT_ADC1(__CHANNEL__)                           \
  (((__CHANNEL__) == ADC_CHANNEL_1)    ? ADC_CHANNEL_0                         \
   ...
   : ((__CHANNEL__) == ADC_CHANNEL_18) ? ADC_CHANNEL_19                        \
                                       : 0UL)
 ...
 * channel.
 */
#define ADC_CHANNEL_DIFF_NEG_INPUT_ADC2(__CHANNEL__)                           \
  (((__CHANNEL__) == ADC_CHANNEL_1)    ? ADC_CHANNEL_0                         \
   : ((__CHANNEL__) == ADC_CHANNEL_2)  ? ADC_CHANNEL_6                         \
   : ((__CHANNEL__) == ADC_CHANNEL_3)  ? ADC_CHANNEL_7                         \
   : ((__CHANNEL__) == ADC_CHANNEL_4)  ? ADC_CHANNEL_8                         \
 ...
 * channel.
 */
#define ADC_CHANNEL_DIFF_NEG_INPUT_ADC3(__CHANNEL__)                           \
  (((__CHANNEL__) == ADC_CHANNEL_1)    ? ADC_CHANNEL_0                         \
   : ((__CHANNEL__) == ADC_CHANNEL_2)  ? ADC_CHANNEL_6                         \
   : ((__CHANNEL__) == ADC_CHANNEL_3)  ? ADC_CHANNEL_7                         \
   : ((__CHANNEL__) == ADC_CHANNEL_4)  ? ADC_CHANNEL_8                         \
   ...
   : ((__CHANNEL__) == ADC_CHANNEL_14) ? ADC_CHANNEL_15                        \
                                       : 0UL)
#endif /* ADC_VER_V5_V90 */
 ...
 * channel.
 */
#define ADC_CHANNEL_DIFF_NEG_INPUT(__HANDLE__, __CHANNEL__)                    \
  ((((__HANDLE__)->Instance) == ADC1)                                          \
       ? ADC_CHANNEL_DIFF_NEG_INPUT_ADC1(__CHANNEL__)                          \
   : (((__HANDLE__)->Instance) == ADC2)                                        \
       ? ADC_CHANNEL_DIFF_NEG_INPUT_ADC2(__CHANNEL__)                          \
   : (((__HANDLE__)->Instance) == ADC3)                                        \
       ? ADC_CHANNEL_DIFF_NEG_INPUT_ADC3(__CHANNEL__)                          \
       : 0UL)
#else
/**
 * @brief  Helper macro to determine the selected channel corresponding
...
HAL_ADCEx_LinearCalibration_SetValue(ADC_HandleTypeDef *hadc,
                                     uint32_t *LinearCalib_Buffer);
HAL_StatusTypeDef
HAL_ADCEx_LinearCalibration_FactorLoad(ADC_HandleTypeDef *hadc);
