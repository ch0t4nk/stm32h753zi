...
void l6470_fault_monitor_flag_interrupt(void) {
  ...
  GPIO_PinState flag_state = HAL_GPIO_ReadPin(MOTOR_FLAG_PORT, MOTOR_FLAG_PIN);
  ...
  if (flag_state == GPIO_PIN_RESET) {
    ...
    for (uint8_t motor_id = 0; motor_id < MAX_MOTORS; motor_id++) {
      ...
      if (result == SYSTEM_OK) {
        L6470HwFaultType_t fault_type = decode_l6470_status(status_register);
...
void l6470_fault_monitor_busy_interrupt(void) {
  ...
  GPIO_PinState busy_state = HAL_GPIO_ReadPin(MOTOR_BUSY_PORT, MOTOR_BUSY_PIN);
...
static L6470HwFaultType_t decode_l6470_status(uint16_t status_register) {
  ...
  if (status_register & (L6470_STATUS_STEP_LOSS_A | L6470_STATUS_STEP_LOSS_B)) {
    fault_type |= L6470_HW_FAULT_STEP_LOSS;
  }
  if (status_register & (L6470_STATUS_WRONG_CMD | L6470_STATUS_NOTPERF_CMD)) {
