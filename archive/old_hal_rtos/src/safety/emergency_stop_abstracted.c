...
#include <string.h>
// Add stdio for debug traces in host testing
#include <stdio.h>
...
SystemError_t emergency_stop_init(void) {
    printf("[arch_estop] emergency_stop_init() start\n");
    // Reset context
    ...
    if (result != SYSTEM_OK) {
        printf("[arch_estop] estop_init_hardware() failed with %d\n",
               (int)result);
        fflush(stdout);
        return result;
    }
    ...
    printf("[arch_estop] emergency_stop_init() complete, state=ARMED\n");
    fflush(stdout);
...
static SystemError_t estop_init_hardware(void) {
    ...
    printf("[arch_estop] estop_init_hardware()\n");
    fflush(stdout);
    ...
    printf("[arch_estop] HAL_Abstraction_GPIO_Init(button) -> %d\n",
           (int)result);
    fflush(stdout);
    if (result != SYSTEM_OK)
        return result;
    ...
    result = HAL_Abstraction_GPIO_EnableInterrupt(
        ...
        0); // Highest priority
    printf("[arch_estop] HAL_Abstraction_GPIO_EnableInterrupt -> %d\n",
           (int)result);
    fflush(stdout);
    if (result != SYSTEM_OK)
        return result;
    ...
    printf("[arch_estop] HAL_Abstraction_GPIO_Init(LED) -> %d\n", (int)result);
    fflush(stdout);
    if (result != SYSTEM_OK)
        return result;
    ...
    printf("[arch_estop] HAL_Abstraction_GPIO_Init(RELAY1) -> %d\n",
           (int)result);
    fflush(stdout);
    if (result != SYSTEM_OK)
        return result;
    ...
    printf("[arch_estop] HAL_Abstraction_GPIO_Init(RELAY2) -> %d\n",
           (int)result);
    fflush(stdout);
