...
typedef struct {
  ...
  FunctionalState Watchdog; /*!< Enables or disables the Watchdog timer on Rx
                               ...
                               2048 bytes to be received. When disabled, the MAC
                               can receive up to 16384 bytes. */

  FunctionalState
      Jabber; /*!< Enables or disables Jabber timer on Tx path
                         When enabled, the MAC allows no more then 2048 bytes to
                 be sent. When disabled, the MAC can send up to 16384 bytes. */
...
#endif
{
  ...
  __IO HAL_ETH_StateTypeDef gState; /*!< ETH state information related to global
                                       Handle management and also related to Tx
                                       operations. This parameter can be a value
                                       of @ref HAL_ETH_StateTypeDef */
...
#define ETH_DMATXNDESCRF_CPC_CRC_INSERT                                        \
  ((uint32_t)0x04000000U) /*!< CRC Pad Control: CRC Insertion (Disable Pad     \
                             Insertion) */
...
#define ETH_DMATXNDESCRF_SAIC_DISABLE                                          \
  ((uint32_t)0x00000000U) /*!< SA Insertion Control: Do not include the source \
                             address */
#define ETH_DMATXNDESCRF_SAIC_INSERT                                           \
  ((uint32_t)0x00800000U) /*!< SA Insertion Control: Include or insert the     \
                             source address */
#define ETH_DMATXNDESCRF_SAIC_REPLACE                                          \
  ((uint32_t)0x01000000U) /*!< SA Insertion Control: Replace the source        \
                             address */
...
#define ETH_DMATXNDESCRF_CIC_IPHDR_INSERT                                      \
  ((uint32_t)0x00010000U) /*!< Only IP header checksum calculation and         \
                             insertion are enabled. */
#define ETH_DMATXNDESCRF_CIC_IPHDR_PAYLOAD_INSERT                              \
  ((uint32_t)0x00020000U) /*!< IP header checksum and payload checksum         \
                             calculation and insertion are enabled, but pseudo \
                             header checksum is not calculated in hardware */
#define ETH_DMATXNDESCRF_CIC_IPHDR_PAYLOAD_INSERT_PHDR_CALC                    \
  ((uint32_t)0x00030000U) /*!< IP Header checksum and payload checksum         \
                             calculation and insertion are enabled, and pseudo \
                             header checksum is calculated in hardware. */
...
#define ETH_DMATXNDESCWBF_TTSL                                                 \
  ((uint32_t)0xFFFFFFFFU) /*!< Buffer1 Address Pointer or TSO Header Address   \
                             Pointer */
...
#define ETH_DMATXNDESCWBF_ES                                                   \
  ((uint32_t)0x00008000U) /*!< Error summary: OR of the following bits: IHE || \
                             UF || ED || EC || LCO || PCE || NC || LCA || FF   \
                             || JT */
#define ETH_DMATXNDESCWBF_JT ((uint32_t)0x00004000U) /*!< Jabber Timeout */
#define ETH_DMATXNDESCWBF_FF                                                   \
  ((uint32_t)0x00002000U) /*!< Packet Flushed: DMA/MTL flushed the packet due  \
                             to SW flush */
...
#define ETH_DMATXNDESCWBF_EC                                                   \
  ((uint32_t)0x00000100U) /*!< Excessive Collision: transmission aborted after \
                             16 collisions */
...
#define ETH_DMATXNDESCWBF_UF                                                   \
  ((uint32_t)0x00000004U) /*!< Underflow Error: late data arrival from the     \
                             memory */
...
#define ETH_DMATXCDESC_TCMSSV                                                  \
  ((uint32_t)0x04000000U) /*!< One-Step Timestamp Correction Input or MSS      \
                             Valid */
...
#define ETH_DMATXCDESC_IVTIR_REMOVE                                            \
  ((uint32_t)0x00040000U) /*!< Remove the inner VLAN tag from the packets      \
                             before transmission. */
...
#define ETH_DMARXNDESCWBF_PMT_RESERVED                                         \
  ((uint32_t)0x00000F00U) /*!< PTP Message Type: PTP packet with Reserved      \
                             message type  */
...
#define ETH_DMARXNDESCWBF_L3L4FM                                               \
  ((uint32_t)0x20000000U) /*!< L3 and L4 Filter Number Matched: if reset       \
                             filter 0 is matched , if set filter 1 is matched  \
                           */
...
#define ETH_DMARXNDESCWBF_LT_DVLAN                                             \
  ((uint32_t)0x00050000U) /*!< The packet is a type packet with Double VLAN    \
                             Tag */
...
HAL_StatusTypeDef HAL_ETH_DescAssignMemory(ETH_HandleTypeDef *heth,
                                           uint32_t Index, uint8_t *pBuffer1,
                                           uint8_t *pBuffer2);
...
HAL_StatusTypeDef
HAL_ETH_GetMACFilterConfig(ETH_HandleTypeDef *heth,
