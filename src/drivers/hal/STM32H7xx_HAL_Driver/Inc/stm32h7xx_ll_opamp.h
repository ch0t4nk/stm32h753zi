...
#define LL_OPAMP_INPUT_NONINVERT_DAC                                           \
  OPAMP_CSR_VPSEL_0 /*!< OPAMP non inverting input connected internally to DAC \
                       channel (DAC1_CH1 for OPAMP1, DAC1_CH2  for OPAMP2)     \
...
#define LL_OPAMP_INPUT_NONINVERT_DAC2                                          \
  OPAMP_CSR_VPSEL_1 /*!< OPAMP non inverting input connected internally to     \
                       DAC2 channel (DAC3 only for OPAMP2)*/
#endif              /* DAC2 */
...
#define LL_OPAMP_INPUT_INVERT_CONNECT_NO                                       \
  OPAMP_CSR_VMSEL_1 /*!< OPAMP inverting input not externally connected        \
                       ...
                       series, this literal include cases of value 0x11 for    \
...
#define LL_OPAMP_TRIMMING_NMOS_VREF_90PC_VDDA                                  \
  (OPAMP_OTR_TRIMOFFSETN |                                                     \
   ((OPAMP_CSR_CALSEL_1 | OPAMP_CSR_CALSEL_0)                                  \
    << 4)) /*!< OPAMP trimming of transistors differential pair NMOS (internal \
              reference voltage set to 0.9*Vdda). Default parameters to be     \
              used for calibration using two trimming steps (one with each     \
...
#define LL_OPAMP_TRIMMING_PMOS_VREF_10PC_VDDA                                  \
  (OPAMP_OTR_TRIMOFFSETP |                                                     \
   (OPAMP_CSR_CALSEL_0                                                         \
    << 4)) /*!< OPAMP trimming of transistors differential pair PMOS (internal \
              reference voltage set to 0.1*Vdda). Default parameters to be     \
              used for calibration using two trimming steps (one with each     \
              transistors differential pair NMOS and PMOS). */
#define LL_OPAMP_TRIMMING_PMOS_VREF_3_3PC_VDDA                                 \
  (OPAMP_OTR_TRIMOFFSETP) /*!< OPAMP trimming of transistors differential pair \
