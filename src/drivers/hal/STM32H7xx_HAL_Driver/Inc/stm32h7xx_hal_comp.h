...
typedef struct {
  ...
  uint32_t WindowMode; /*!< Set window mode of a pair of comparators instances
                          ...
                          COMP<x+1>). Note: HAL COMP driver allows to set window
                          mode from any COMP instance of the pair of COMP
                          instances composing window mode. This parameter can be
                          a value of @ref COMP_WindowMode */
...
#define COMP_WINDOWMODE_DISABLE                                                \
  ((uint32_t)0x00000000) /*!< Window mode disable: Comparators instances pair  \
                            COMP1 and COMP2 are independent */
...
#define COMP_INPUT_PLUS_IO1                                                    \
  ((uint32_t)0x00000000) /*!< Comparator input plus connected to IO1 (pin PB0  \
                            for COMP1, pin PE9 for COMP2) */
...
#define COMP_OUTPUTPOL_NONINVERTED                                             \
  ((uint32_t)0x00000000) /*!< COMP output level is not inverted (comparator    \
...
#define COMP_BLANKINGSRC_TIM1_OC5                                              \
  (COMP_CFGRx_BLANKING_0) /*!< TIM1 OC5 selected as blanking source for        \
                             comparator */
#define COMP_BLANKINGSRC_TIM2_OC3                                              \
  (COMP_CFGRx_BLANKING_1) /*!< TIM2 OC3 selected as blanking source for        \
                             comparator */
#define COMP_BLANKINGSRC_TIM3_OC3                                              \
  (COMP_CFGRx_BLANKING_0 |                                                     \
   COMP_CFGRx_BLANKING_1) /*!< TIM3 OC3 selected as blanking source for        \
                             compartor */
#define COMP_BLANKINGSRC_TIM3_OC4                                              \
  (COMP_CFGRx_BLANKING_2) /*!< TIM3 OC4 selected as blanking source for        \
                             comparator */
#define COMP_BLANKINGSRC_TIM8_OC5                                              \
  (COMP_CFGRx_BLANKING_2 |                                                     \
   COMP_CFGRx_BLANKING_0) /*!< TIM8 OC5 selected as blanking source for        \
                             comparator */
#define COMP_BLANKINGSRC_TIM15_OC1                                             \
  (COMP_CFGRx_BLANKING_2 |                                                     \
   COMP_CFGRx_BLANKING_1) /*!< TIM15 OC1 selected as blanking source for       \
                             comparator */
/**
 * @}
 */
...
#define COMP_TRIGGERMODE_NONE                                                  \
  ((uint32_t)0x00000000) /*!< Comparator output triggering no External         \
                            Interrupt Line */
...
#define __HAL_COMP_IS_LOCKED(__HANDLE__)                                       \
  (READ_BIT((__HANDLE__)->Instance->CFGR, COMP_CFGRx_LOCK) == COMP_CFGRx_LOCK)
...
#define __HAL_COMP_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__)                    \
  ((((__HANDLE__)->Instance->CFGR & (__INTERRUPT__)) == (__INTERRUPT__))       \
       ? SET                                                                   \
       : RESET)
