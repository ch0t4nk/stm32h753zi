...
static SPI_HandleTypeDef* l6470_spi_handle = NULL;
...
static SystemError_t l6470_spi_transaction(uint8_t* tx_data, uint8_t* rx_data, uint16_t size);
static SystemError_t l6470_send_daisy_command(uint8_t motor1_cmd, uint32_t motor1_data, 
                                              uint8_t motor2_cmd, uint32_t motor2_data);
static SystemError_t l6470_send_single_command(uint8_t motor_id, uint8_t command, uint32_t data);
static uint32_t l6470_pack_parameter(uint8_t register_addr, uint32_t value, uint8_t num_bytes);
static void l6470_unpack_parameter(uint8_t* rx_data, uint32_t* value, uint8_t num_bytes);
...
SystemError_t l6470_init(SPI_HandleTypeDef* spi_handle) {
    ...
    l6470_spi_handle = spi_handle;
    
    // Initialize CS pin (ensure HIGH before first communication)
    HAL_GPIO_WritePin(L6470_CS_PORT, L6470_CS_PIN, GPIO_PIN_SET);
...
SystemError_t l6470_init_motor(uint8_t motor_id) {
    ...
    result = l6470_set_parameter(motor_id, L6470_REG_MAX_SPEED, L6470_MAX_SPEED);
    if (result != SYSTEM_OK) return result;
    
    result = l6470_set_parameter(motor_id, L6470_REG_MIN_SPEED, L6470_MIN_SPEED);
    if (result != SYSTEM_OK) return result;
    ...
    result = l6470_set_parameter(motor_id, L6470_REG_KVAL_HOLD, L6470_KVAL_HOLD);
    if (result != SYSTEM_OK) return result;
    ...
    // Configure step mode
    result = l6470_set_parameter(motor_id, L6470_REG_STEP_MODE, L6470_STEP_MODE);
    if (result != SYSTEM_OK) return result;
    ...
    state->last_command_time = HAL_GetTick();
...
SystemError_t l6470_set_parameter(uint8_t motor_id, uint8_t register_addr, uint32_t value) {
    ...
    uint32_t packed_data = l6470_pack_parameter(register_addr, value, param_bytes);
...
SystemError_t l6470_get_status(uint8_t motor_id, uint16_t* status) {
    ...
    // Assert CS
    HAL_GPIO_WritePin(L6470_CS_PORT, L6470_CS_PIN, GPIO_PIN_RESET);
    
    // Small delay for L6470 setup time (from datasheet)
    HAL_Delay(1); // TODO: Implement microsecond delay function
    
    HAL_StatusTypeDef spi_result = HAL_SPI_TransmitReceive(l6470_spi_handle, tx_data, rx_data, 2, SPI_TIMEOUT_MS);
...
SystemError_t l6470_move_to_position(uint8_t motor_id, int32_t position) {
    ...
    if (result == SYSTEM_OK) {
        ...
        driver_state[motor_id].last_command_time = HAL_GetTick();
    }
 ...
 */
static SystemError_t l6470_spi_transaction(uint8_t* tx_data, uint8_t* rx_data, uint16_t size) {
    if (l6470_spi_handle == NULL || tx_data == NULL || rx_data == NULL) {
        return ERROR_MOTOR_CONFIG_INVALID;
    }
    ...
    HAL_Delay(1); // TODO: Replace with microsecond delay
    
    HAL_StatusTypeDef result = HAL_SPI_TransmitReceive(l6470_spi_handle, tx_data, rx_data, size, SPI_TIMEOUT_MS);
 ...
 * @return System error code
 */
SystemError_t l6470_get_driver_state(uint8_t motor_id, L6470_DriverState_t* state) {
    if (state == NULL) {
        return ERROR_MOTOR_CONFIG_INVALID;
    }
...
SystemError_t l6470_run(uint8_t motor_id, bool direction, float speed) {
    ...
    if (motor_id == 0) {
        ...
        return l6470_send_daisy_command(command, l6470_speed, L6470_CMD_NOP, 0);
    } else {
        // Motor 0: NOP, Motor 1: send command  
        return l6470_send_daisy_command(L6470_CMD_NOP, 0, command, l6470_speed);
    }
}
...
SystemError_t l6470_reset_position(uint8_t motor_id) {
    ...
    if (motor_id == 0) {
        // Motor 0: send command, Motor 1: NOP
        return l6470_send_daisy_command(L6470_CMD_RESET_POS, 0x00, L6470_CMD_NOP, 0x00);
    } else {
        // Motor 0: NOP, Motor 1: send command
        return l6470_send_daisy_command(L6470_CMD_NOP, 0x00, L6470_CMD_RESET_POS, 0x00);
    }
}
