 ...
 */
void MockHAL_Reset(void) { HAL_Abstraction_Mock_Reset(); }
...
void MockHAL_InjectFault(uint32_t fault_type, bool enable) {
  ...
  if (fault_type & MOCK_FAULT_GPIO_WRITE) {
    mock_hal_state.inject_gpio_failure = enable; // Same as GPIO_INIT for now
  }
...
HAL_Abstraction_SPI_TransmitReceive(HAL_SPI_Instance_t instance,
  ...
  if (transaction->tx_data &&
    ...
    memcpy(spi->last_tx_data, transaction->tx_data, transaction->data_size);
  }
  ...
  if (transaction->rx_data &&
    ...
    for (uint16_t i = 0; i < transaction->data_size; i++) {
      transaction->rx_data[i] = HAL_MOCK_TEST_PATTERN_BASE + i; // Test pattern
    }
    memcpy(spi->last_rx_data, transaction->rx_data, transaction->data_size);
  }
...
HAL_Abstraction_I2C_MemRead(HAL_I2C_Instance_t instance,
  ...
  if (transaction->data && transaction->data_size <= sizeof(i2c->last_data)) {
    ...
    if (transaction->device_address == 0x36) {     // AS5600 address
      ...
      } else if (transaction->register_address == 0x0E) { // ANGLE register
...
uint32_t HAL_Abstraction_GetTick(void) { return mock_hal_state.system_tick; }
