...
SystemError_t motion_profile_generate_trapezoidal(MotionProfile_t* profile,
    ...
    if (2 * accel_distance <= total_distance) {
        ...
        profile->const_vel_time_ms = (uint32_t)((float)profile->const_vel_distance / max_vel * 1000);
        profile->decel_time_ms = profile->accel_time_ms;
        profile->total_time_ms = profile->accel_time_ms + profile->const_vel_time_ms + profile->decel_time_ms;
...
SystemError_t motion_profile_generate_scurve(MotionProfile_t* profile,
    ...
    profile->linear_accel_time_ms = (uint32_t)((float)max_vel / max_accel * 1000) - jerk_time;
    ...
    profile->total_time_ms = 2 * (profile->jerk_accel_time_ms + 
...
SystemError_t motion_profile_execute(uint8_t motor_id,
    ...
    if (motor_id >= MAX_MOTORS || profile == NULL || 
        target_pos == NULL || target_vel == NULL) {
        return ERROR_INVALID_PARAMETER;
    }
    ...
    switch (profile->type) {
        case PROFILE_TRAPEZOIDAL:
            return execute_trapezoidal_profile(profile, elapsed_time_ms, target_pos, target_vel);
            
        case PROFILE_SCURVE:
            return execute_scurve_profile(profile, elapsed_time_ms, target_pos, target_vel);
...
static SystemError_t execute_trapezoidal_profile(MotionProfile_t* profile,
    ...
    if (elapsed_time_ms <= profile->accel_time_ms) {
        ...
        *target_vel = (uint32_t)(profile->acceleration * elapsed_sec);
        float distance = 0.5f * profile->acceleration * elapsed_sec * elapsed_sec;
        *target_pos = profile->start_position + (int32_t)(distance * profile->direction);
        
    } else if (elapsed_time_ms <= (profile->accel_time_ms + profile->const_vel_time_ms)) {
        ...
        *target_vel = profile->peak_velocity;
        ...
        float distance = profile->accel_distance + (profile->peak_velocity * const_vel_elapsed);
        *target_pos = profile->start_position + (int32_t)(distance * profile->direction);
        
    } else {
        // Deceleration phase
        profile->current_phase = PROFILE_PHASE_DECEL;
        float decel_elapsed = elapsed_sec - accel_time_sec - const_vel_time_sec;
        *target_vel = (uint32_t)(profile->peak_velocity - (profile->deceleration * decel_elapsed));
        
        if (*target_vel > profile->peak_velocity) {
            *target_vel = 0; // Handle underflow
        }
        
        float decel_distance = profile->peak_velocity * decel_elapsed - 
                              0.5f * profile->deceleration * decel_elapsed * decel_elapsed;
        float total_distance = profile->accel_distance + profile->const_vel_distance + decel_distance;
        *target_pos = profile->start_position + (int32_t)(total_distance * profile->direction);
    }
...
static SystemError_t execute_scurve_profile(MotionProfile_t* profile,
    ...
    if (elapsed_time_ms <= profile->jerk_accel_time_ms) {
        ...
        *target_vel = (uint32_t)(jerk_accel);
        float distance = (1.0f/6.0f) * profile->jerk * elapsed_sec * elapsed_sec * elapsed_sec;
        *target_pos = profile->start_position + (int32_t)(distance * profile->direction);
        ...
        float jerk_velocity = 0.5f * profile->jerk * jerk_time_sec * jerk_time_sec;
        *target_vel = (uint32_t)(jerk_velocity + profile->acceleration * remaining_accel_time);
        
        // Simplified position calculation
        float jerk_distance = (1.0f/6.0f) * profile->jerk * jerk_time_sec * jerk_time_sec * jerk_time_sec;
        float linear_distance = jerk_velocity * remaining_accel_time + 
                               0.5f * profile->acceleration * remaining_accel_time * remaining_accel_time;
        *target_pos = profile->start_position + (int32_t)((jerk_distance + linear_distance) * profile->direction);
    }
 ...
 * @return SystemError_t Operation result
 */
SystemError_t motion_profile_get_status(uint8_t motor_id, MotionProfileStatus_t* status) {
...
SystemError_t motion_profile_synchronize(uint8_t* motor_ids,
                                        ...
                                        uint8_t motor_count) {
    if (motor_ids == NULL || profiles == NULL || motor_count == 0 || motor_count > MAX_MOTORS) {
        return ERROR_INVALID_PARAMETER;
    }
    ...
    for (uint8_t i = 0; i < motor_count; i++) {
        if (profiles[i].total_time_ms < max_time) {
            ...
            profiles[i].max_velocity = (uint32_t)(profiles[i].max_velocity * scale_factor);
            profiles[i].peak_velocity = (uint32_t)(profiles[i].peak_velocity * scale_factor);
            
            // Recalculate timing with new velocity
            motion_profile_generate_trapezoidal(&profiles[i],
                                               profiles[i].start_position,
                                               profiles[i].end_position,
