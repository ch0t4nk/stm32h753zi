...
/* Provide a host-friendly no-op macro for busy-wait loops when building
 * on non-ARM hosts. On target builds CMSIS provides a proper __NOP
 * implementation; for host tests we use a harmless empty expression.
 */
#ifndef __NOP
#define __NOP() ((void)0)
#endif
