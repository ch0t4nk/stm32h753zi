...
SystemError_t multi_motor_coordinated_move(CoordinatedMoveCommand_t *move_cmd) {
  ...
  // Store move command
  memcpy(&coordinator.current_move, move_cmd, sizeof(CoordinatedMoveCommand_t));
...
SystemError_t multi_motor_get_status(CoordinationStatus_t *status) {
  ...
  status->load_balance_enabled = coordinator.load_config.load_balance_enabled;

  // Copy motor states
  for (uint8_t i = 0; i < MAX_MOTORS; i++) {
    status->motor_enabled[i] = coordinator.motor_states[i].enabled;
    status->motor_synchronized[i] = coordinator.motor_states[i].synchronized;
...
execute_synchronized_move(CoordinatedMoveCommand_t *move_cmd) {
  ...
  for (uint8_t i = 0; i < MAX_MOTORS; i++) {
    if (move_cmd->motor_targets[i].enabled &&
      ...
      uint32_t move_time =
          calculate_move_time(distance, move_cmd->motor_targets[i].max_velocity,
...
execute_load_sharing_move(CoordinatedMoveCommand_t *move_cmd) {
  ...
  for (uint8_t i = 0; i < MAX_MOTORS; i++) {
    if (move_cmd->motor_targets[i].enabled &&
      ...
      int32_t target_pos =
          pos_status.current_position +
          (load_per_motor * ((move_cmd->motor_targets[i].target_position >
                              pos_status.current_position)
                                 ? 1
                                 : -1));
      ...
      profile_config.max_velocity = move_cmd->motor_targets[i].max_velocity;
      profile_config.acceleration = move_cmd->motor_targets[i].acceleration;
      profile_config.direction = (target_pos > pos_status.current_position)
                                     ? DIRECTION_FORWARD
                                     : DIRECTION_BACKWARD;
...
execute_master_slave_move(CoordinatedMoveCommand_t *move_cmd) {
  ...
  if (move_cmd->motor_targets[master_id].enabled &&
    ...
    MotionProfile_t master_config = {
        .type = PROFILE_TRAPEZOIDAL,
        .start_position = coordinator.motor_states[master_id].current_position,
  ...
  for (uint8_t i = 0; i < MAX_MOTORS; i++) {
    if (coordinator.motor_states[i].enabled) {
      ...
      if (position_control_get_status(i, &pos_status) == SYSTEM_OK) {
        coordinator.motor_states[i].current_position =
            pos_status.current_position;
        coordinator.motor_states[i].settled = pos_status.position_settled;
      }

      // Update motion profile status
      coordinator.motor_states[i].profile_active = motion_profile_is_active(i);
    }
  }
}
...
static void update_synchronized_motion(uint32_t dt_ms) {
  ...
  for (uint8_t i = 0; i < MAX_MOTORS; i++) {
    if (coordinator.motor_states[i].synchronized &&
      ...
      uint8_t master_id = coordinator.sync_config.sync_master;
      int32_t sync_error = coordinator.motor_states[i].current_position -
                           coordinator.motor_states[master_id].current_position;
  ...
  for (uint8_t i = 0; i < MAX_MOTORS; i++) {
    if (coordinator.motor_states[i].load_share_enabled &&
      ...
      int32_t position_diff =
          coordinator.motor_states[i].current_position - average_position;

      if (abs(position_diff) > coordinator.load_config.max_load_difference) {
        // Adjust this motor's target to balance load
        int32_t adjustment = position_diff / 2; // Move halfway back to average
        int32_t new_target =
...
static uint32_t calculate_move_time(int32_t distance, uint32_t max_velocity,
  ...
  float accel_distance = 0.5f * (float)acceleration * accel_time * accel_time;
