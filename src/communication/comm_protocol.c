 ...
 * @brief Communication Protocol Implementation - STM32H753ZI Stepper Motor
 * Project
...
static CommChannelConfig_t comm_channels[7] = {
    0}; // One for each protocol type

// UART communication state
// NOTE: DMA Traceability (concrete mapping)
// See: docs/README-peripherals.md for high-level mapping. The repository
// contains an implementation example and MX/HAL guidance that binds
// USART3 to DMA streams as shown below (used as the canonical mapping for
// this project unless board MX overrides are present):
//
//  USART3 RX -> hdma_usart3_rx
//    - DMA Instance: DMA1_Stream1
//    - DMA Request: DMA_REQUEST_USART3_RX
//    - Mode: DMA_CIRCULAR (recommended for continuous RX)
//    - Link: __HAL_LINKDMA(&huart3, hdmarx, hdma_usart3_rx)
//
//  USART3 TX -> hdma_usart3_tx
//    - DMA Instance: DMA1_Stream3
//    - DMA Request: DMA_REQUEST_USART3_TX
//    - Mode: DMA_NORMAL
//    - Link: __HAL_LINKDMA(&huart3, hdmatx, hdma_usart3_tx)
//
// Provenance / where to verify in this repo (authoritative anchors):
//  - `src/application/uart_test_fixed.c` — HAL_UART_MspInit implementation for
//    USART3 (GPIO + clock). Function starts at ~line 284 in that file.
//  - `archive/old_hal_rtos/Core/Src/stm32h7xx_hal_msp.c` — archived board MSP
//    implementation including `HAL_UART_MspInit` for USART3. Function starts
//    at ~line 360.
//  - Archive example mapping with DMA bindings:
//    `archive/development_artifacts/instruction_archive/granular_instructions/stm32h7-uart-protocol.instructions.md`
//    (search for `hdma_usart3_rx.Instance = DMA1_Stream1`,
//    `hdma_usart3_tx.Instance = DMA1_Stream3`, and
//    `__HAL_LINKDMA(&huart3, hdmarx, hdma_usart3_rx`) — concrete examples
//    around lines 122-136 and DMA head usage around line 184.
//  - Runtime verification: `src/application/uart_verification_test.c` checks
//    for `huart3.hdmatx` / `huart3.hdmarx` presence during host tests.
//
// If your board's MX-generated code differs, update the mapping here and
// place exact MX file references in `docs/README-peripherals.md`.
...
SystemError_t comm_protocol_init(void) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        comm_channels[i].last_activity = HAL_Abstraction_GetTick();
    }
...
SystemError_t comm_uart_init(UART_HandleTypeDef *huart,
    ...
    channel->last_activity = HAL_Abstraction_GetTick();

    // Start UART DMA reception
    if (HAL_UART_Receive_DMA(huart, uart_rx_buffer, UART_RX_BUFFER_SIZE) !=
        HAL_OK) {
...
SystemError_t comm_process_message(const Message_t *message) {
    ...
    uint16_t calculated_checksum =
        calculate_message_checksum(&message->header, message->payload);
...
SystemError_t comm_process_ascii_command(const char *command_string) {
    ...
    SystemError_t result =
        comm_parse_ascii_motor_command(command_string, &motor_command);
...
SystemError_t comm_parse_ascii_motor_command(const char *ascii_command,
    ...
    int params = sscanf(ascii_command, "%15s %15s %15s %15s", command, param1,
                        param2, param3);
    ...
    if (strcmp(command, "MOVE") == 0) {
        if (params >= 4) {
            ...
            motor_command->data.move.acceleration =
                MOTOR1_ACCELERATION; // Default from SSOT
        } else {
            return ERROR_COMM_INVALID_COMMAND;
        }
...
bool comm_channel_is_healthy(CommProtocol_t protocol) {
    ...
    bool recent_activity =
        (current_time - channel->last_activity) < (channel->timeout_ms * 2);
    bool low_error_rate =
        (channel->error_count * 10) < (channel->tx_count + channel->rx_count);

    return channel->enabled &&
           (recent_activity || (channel->tx_count + channel->rx_count) == 0) &&
           low_error_rate;
}
...
SystemError_t comm_protocol_self_test(void) {
    ...
    if (debug_uart_handle != NULL &&
        comm_channels[PROTOCOL_UART_ASCII].enabled) {
        const char *test_message = "TEST\r\n";
        SystemError_t result =
            comm_send_text_message(PROTOCOL_UART_ASCII, test_message);
        if (result != SYSTEM_OK) {
    ...
    MotorCommand_t test_command = {
        .motor_id = 0, .command = MOTOR_CMD_GET_STATUS, .data.raw_data = 0};
...
void comm_uart_rx_complete_callback(UART_HandleTypeDef *huart) {
    if (huart == debug_uart_handle) {
        uart_rx_length =
            UART_RX_BUFFER_SIZE - __HAL_DMA_GET_COUNTER(huart->hdmarx);
        uart_rx_complete = true;
        comm_channels[PROTOCOL_UART_ASCII].rx_count++;
        comm_channels[PROTOCOL_UART_ASCII].last_activity =
...
void comm_uart_error_callback(UART_HandleTypeDef *huart) {
    if (huart == debug_uart_handle) {
        ...
        fault_monitor_record_system_fault(
            SYSTEM_FAULT_UART_FAULT, FAULT_SEVERITY_WARNING, huart->ErrorCode);
...
static SystemError_t process_motor_command(const MotorCommand_t *command) {
    ...
    case MOTOR_CMD_MOVE_ABSOLUTE: {
        ...
        float target_deg = MOTOR_CONTROLLER_STEPS_TO_DEGREES(
            command->data.move.position_steps);
        result =
            motor_controller_move_to_position(command->motor_id, target_deg);
        break;
    }
...
static SystemError_t validate_motor_command(const MotorCommand_t *command) {
    ...
    case MOTOR_CMD_MOVE_RELATIVE:
        ...
        if (abs(command->data.move.position_steps) >
            MOTOR_MAX_POSITION_STEPS) {
            return ERROR_MOTOR_POSITION_LIMIT;
        }
        // Validate speed limits
        if (command->data.move.speed_steps_per_sec >
    ...
    case MOTOR_CMD_SET_PARAMETERS:
        ...
        if (command->data.parameters.max_speed >
                MOTOR_MAX_SPEED_STEPS_PER_SEC ||
...
static SystemError_t send_uart_message(const uint8_t *data, uint32_t length) {
    ...
    if (HAL_UART_Transmit_DMA(debug_uart_handle, (uint8_t *)data, length) !=
        HAL_OK) {
        uart_tx_complete = true;
...
static SystemError_t process_uart_received_data(void) {
    ...
    for (uint32_t i = 0; i < uart_rx_length; i++) {
        ...
        if (received_char == ASCII_COMMAND_DELIMITER ||
            received_char == ASCII_END_OF_LINE) {
