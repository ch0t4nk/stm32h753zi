 ...
 * @brief Watchdog Management System Implementation - STM32H753ZI Stepper Motor Project
...
SystemError_t watchdog_self_test(void) {
    ...
    uint32_t refresh_time = HAL_GetTick() - test_start;
    
    if (refresh_result != SYSTEM_OK || refresh_time > 5) {  // Should be very fast
        return ERROR_SAFETY_SELF_TEST_FAILED;
    }
...
bool watchdog_refresh_due(void) {
    ...
    uint32_t elapsed = HAL_GetTick() - watchdog_config.last_refresh;
...
static void calculate_watchdog_timing(void) {
    ...
    uint32_t prescaler_value = 4 << ((IWDG_PRESCALER >> IWDG_PR_PR_Pos) & 0x07);
    uint32_t calculated_timeout = (4 * prescaler_value * IWDG_RELOAD_VALUE) / 32000;  // Assume 32kHz LSI
