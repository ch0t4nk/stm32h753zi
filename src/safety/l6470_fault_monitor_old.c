 ...
 */
SystemError_t l6470_fault_monitor_get_status(L6470HwFaultStatus_t *status) {ZI Project Team
 * @date 2025-08-07
 *
...
static bool fault_monitor_initialized = false;
     ...
     * @brief Get current L6470 fault status
     */
    SystemError_t l6470_fault_monitor_get_status(L6470FaultStatus_t * status) {
        if (status == NULL) {
    ...
    void l6470_fault_monitor_busy_interrupt(void) {
        ...
        // This can help detect communication issues or unexpected motor
        // behavior
    }
    ...
    static void process_l6470_fault(L6470HwFaultType_t fault_type) {
       ...
       */
    static L6470FaultType_t decode_l6470_status(uint16_t status_register) {
        L6470FaultType_t fault_type = L6470_FAULT_NONE;

        // Check status bits (based on L6470 datasheet)
        if (status_register & L6470_STATUS_OCD) {
            fault_type |= L6470_FAULT_OVERCURRENT;
        }
        if (status_register & L6470_STATUS_TH_SD) {
            fault_type |= L6470_FAULT_THERMAL_SHUTDOWN;
        }
        if (status_register & L6470_STATUS_TH_WRN) {
            fault_type |= L6470_FAULT_THERMAL_WARNING;
        }
        if (status_register & L6470_STATUS_UVLO) {
            fault_type |= L6470_FAULT_UNDERVOLTAGE;
        }
        if (status_register &
            (L6470_STATUS_STEP_LOSS_A | L6470_STATUS_STEP_LOSS_B)) {
            fault_type |= L6470_FAULT_STEP_LOSS;
        }
        if (status_register &
            (L6470_STATUS_WRONG_CMD | L6470_STATUS_NOTPERF_CMD)) {
            fault_type |= L6470_FAULT_COMMAND_ERROR;
        }
