...
SystemError_t emergency_stop_reset(void) {
    ...
    if ((current_time - emergency_stop_state.timestamp) <
        ESTOP_RESET_DELAY_MS) {
...
SystemError_t emergency_stop_self_test(void) {
    ...
    HAL_Delay(1);
    GPIO_PinState relay1_state =
        HAL_GPIO_ReadPin(SAFETY_RELAY1_PORT, SAFETY_RELAY1_PIN);
...
SystemError_t emergency_stop_check_health(void) {
    ...
    if (is_emergency_stop_pressed()) {
        if (button_pressed_time == 0) {
            button_pressed_time = HAL_Abstraction_GetTick();
        } else if ((HAL_Abstraction_GetTick() - button_pressed_time) >
                   30000) { // 30 seconds
