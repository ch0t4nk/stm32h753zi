 ...
 * @brief Safety System Manager Implementation - STM32H753ZI Stepper Motor Project
...
SystemError_t safety_system_init(void) {
    ...
    log_safety_event(SAFETY_EVENT_EMERGENCY_STOP_RESET, 0, get_microsecond_timer());
...
SystemError_t safety_system_task(void) {
    ...
    if ((current_time - last_safety_check_time) >= SAFETY_CHECK_INTERVAL_MS) {
        ...
        if (watchdog_result != SYSTEM_OK) {
            log_safety_event(SAFETY_EVENT_WATCHDOG_WARNING, 0, get_microsecond_timer());
        }
...
SystemError_t reset_emergency_stop(void) {
    ...
    // Log reset event
    log_safety_event(SAFETY_EVENT_EMERGENCY_STOP_RESET, ESTOP_SOURCE_NONE, get_microsecond_timer());
...
static SystemError_t initialize_safety_configurations(void) {
    ...
    safety_functions[SAFETY_FUNC_EMERGENCY_STOP].function = SAFETY_FUNC_EMERGENCY_STOP;
    safety_functions[SAFETY_FUNC_EMERGENCY_STOP].sil_level = SIL_3;
    safety_functions[SAFETY_FUNC_EMERGENCY_STOP].reaction_time_ms = ESTOP_REACTION_TIME_MS;
    ...
    safety_functions[SAFETY_FUNC_SPEED_MONITORING].function = SAFETY_FUNC_SPEED_MONITORING;
    safety_functions[SAFETY_FUNC_SPEED_MONITORING].sil_level = SIL_2;
    safety_functions[SAFETY_FUNC_SPEED_MONITORING].reaction_time_ms = MOTOR_OVERSPEED_TIME_MS;
    safety_functions[SAFETY_FUNC_SPEED_MONITORING].enabled = true;
    safety_functions[SAFETY_FUNC_SPEED_MONITORING].threshold_value = MOTOR_OVERSPEED_THRESHOLD_RPM;
    ...
    safety_functions[SAFETY_FUNC_POSITION_LIMITING].function = SAFETY_FUNC_POSITION_LIMITING;
    ...
    safety_functions[SAFETY_FUNC_POSITION_LIMITING].threshold_value = MOTOR_POSITION_ERROR_MAX_DEG;
...
static SystemError_t initialize_safety_monitors(void) {
    ...
    safety_monitors[MONITOR_MOTOR1_CURRENT].safe_max = MOTOR_OVERCURRENT_THRESHOLD_MA / 1000.0f; // Convert to A
    safety_monitors[MONITOR_MOTOR1_CURRENT].warning_max = safety_monitors[MONITOR_MOTOR1_CURRENT].safe_max * 0.9f;
    safety_monitors[MONITOR_MOTOR1_CURRENT].warning_min = MOTOR_UNDERCURRENT_THRESHOLD_MA / 1000.0f;
    ...
    safety_monitors[MONITOR_MOTOR1_SPEED].safe_min = -MOTOR_OVERSPEED_THRESHOLD_RPM;
    safety_monitors[MONITOR_MOTOR1_SPEED].safe_max = MOTOR_OVERSPEED_THRESHOLD_RPM;
    safety_monitors[MONITOR_MOTOR1_SPEED].warning_max = MOTOR_OVERSPEED_THRESHOLD_RPM * 0.95f;
    safety_monitors[MONITOR_MOTOR1_SPEED].warning_min = -MOTOR_OVERSPEED_THRESHOLD_RPM * 0.95f;
    ...
    safety_monitors[MONITOR_SYSTEM_TEMPERATURE].warning_max = MOTOR_TEMP_WARNING_C;
    safety_monitors[MONITOR_SYSTEM_TEMPERATURE].warning_min = 0.0f;
    safety_monitors[MONITOR_SYSTEM_TEMPERATURE].enabled = true;
    safety_monitors[MONITOR_SYSTEM_TEMPERATURE].current_value = 25.0f; // Assume room temperature
    ...
    safety_monitors[MONITOR_SUPPLY_VOLTAGE].current_value = 12.0f; // Assume nominal voltage
...
static void set_safety_state(SafetyState_t new_state) {
    if (new_state != current_safety_state) {
        ...
        safety_state_entry_time = HAL_GetTick();
...
static void broadcast_emergency_stop(void) {
    ...
    // This would notify all subsystems (motor controllers, communication, etc.)
    // about the emergency stop condition
}

/**
 * @brief Get microsecond timer value
 */
static uint32_t get_microsecond_timer(void) {
    // Using SysTick for microsecond timing
    // STM32H7 runs at 480MHz, SysTick typically configured for 1ms
    ...
    return (tick_ms * 1000) + us_in_tick;
}
