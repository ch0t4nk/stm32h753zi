...
SystemError_t fault_monitor_record_motor_fault(uint8_t motor_id,
    ...
    SystemError_t result =
        add_fault_record(fault_type, severity, motor_id, additional_data);
...
SystemError_t fault_monitor_record_system_fault(SystemFaultType_t fault_type,
    ...
    // Add to fault record
    SystemError_t result =
        add_fault_record(fault_type, severity, 0xFF, additional_data);
...
SystemError_t fault_monitor_clear_fault(uint32_t fault_code) {
    ...
    if (record != NULL && record->active) {
        ...
        // Log safety event
        safety_log_event(SAFETY_EVENT_FAULT_CLEARED, record->motor_id,
                         fault_code);
...
fault_monitor_clear_faults_by_severity(FaultSeverity_t max_severity) {
    ...
    for (uint32_t i = 0; i < total_fault_records; i++) {
        if (fault_records[i].active &&
            fault_records[i].severity <= max_severity) {
            fault_records[i].active = false;
            cleared_count++;

            // Clear from active tracking
            if (fault_records[i].motor_id < MAX_MOTORS) {
                active_motor_faults[fault_records[i].motor_id] &=
                    ~fault_records[i].fault_code;
            } else {
...
SystemError_t fault_monitor_self_test(void) {
    ...
    SystemError_t result = fault_monitor_record_system_fault(
        SYSTEM_FAULT_SELF_TEST, FAULT_SEVERITY_INFO, 0);
    if (result != SYSTEM_OK ||
        fault_config.fault_count != (initial_count + 1)) {
        return ERROR_SAFETY_SELF_TEST_FAILED;
    }
    ...
    if (MOTOR_MAX_CURRENT_MA <= 0 || MOTOR_MAX_SPEED_STEPS_PER_SEC <= 0 ||
        SUPPLY_VOLTAGE_MIN_MV <= 0 ||
        SUPPLY_VOLTAGE_MAX_MV <= SUPPLY_VOLTAGE_MIN_MV) {
        return ERROR_SAFETY_SELF_TEST_FAILED;
    }
...
SystemError_t fault_monitor_check_motor_current(uint8_t motor_id,
    ...
    if (current_ma > MOTOR_MAX_CURRENT_MA) {
        return fault_monitor_record_motor_fault(
            motor_id, MOTOR_FAULT_OVERCURRENT, FAULT_SEVERITY_CRITICAL,
            current_ma);
    }
    ...
    if (current_ma > (MOTOR_MAX_CURRENT_MA * 8 / 10)) {
        return fault_monitor_record_motor_fault(
            motor_id, MOTOR_FAULT_CURRENT_WARNING, FAULT_SEVERITY_WARNING,
            current_ma);
    }
...
SystemError_t fault_monitor_check_motor_speed(uint8_t motor_id,
    ...
    if (speed_steps_per_sec > MOTOR_MAX_SPEED_STEPS_PER_SEC) {
        return fault_monitor_record_motor_fault(
            motor_id, MOTOR_FAULT_SPEED_WARNING, FAULT_SEVERITY_WARNING,
            speed_steps_per_sec);
    }
...
SystemError_t fault_monitor_check_voltage(uint32_t voltage_mv) {
    ...
    if (voltage_mv > SUPPLY_VOLTAGE_MAX_MV) {
        return fault_monitor_record_system_fault(
            SYSTEM_FAULT_POWER_FAILURE, FAULT_SEVERITY_CRITICAL, voltage_mv);
    }
    ...
    if (voltage_mv > (SUPPLY_VOLTAGE_MAX_MV * 9 / 10) ||
        ...
        return fault_monitor_record_system_fault(
            SYSTEM_FAULT_POWER_FAILURE, FAULT_SEVERITY_WARNING, voltage_mv);
    }
...
SystemError_t fault_monitor_check_temperature(int32_t temperature_c) {
    ...
    if (temperature_c > (SYSTEM_MAX_TEMP_C * 9 / 10)) {
        return fault_monitor_record_system_fault(
            SYSTEM_FAULT_POWER_FAILURE, FAULT_SEVERITY_WARNING, temperature_c);
    }
...
SystemError_t fault_monitor_check_position_accuracy(uint8_t motor_id,
    ...
    if (position_error > (MAX_POSITION_ERROR_STEPS / 2)) {
        return fault_monitor_record_motor_fault(
            motor_id, MOTOR_FAULT_ENCODER_WARNING, FAULT_SEVERITY_WARNING,
            position_error);
    }
...
static SystemError_t add_fault_record(uint32_t fault_code,
    ...
    if (existing != NULL) {
        ...
        existing->count++;
        existing->timestamp = HAL_Abstraction_GetTick();
    ...
    record->timestamp = HAL_Abstraction_GetTick();
    record->count = 1;
 ...
 */
__attribute__((unused)) static bool is_fault_critical(uint32_t fault_code) {
    // Check motor fault criticality
    if ((fault_code & MOTOR_FAULT_ALL_MASK) &&
        (fault_code & MOTOR_FAULT_CRITICAL_MASK)) {
        return true;
    }

    // Check system fault criticality
    if ((fault_code & SYSTEM_FAULT_ALL_MASK) &&
        (fault_code & SYSTEM_FAULT_CRITICAL_MASK)) {
        return true;
    }
...
get_fault_description(uint32_t fault_code) {
    // Check motor faults first
    switch (fault_code) {
    case MOTOR_FAULT_OVERCURRENT:
        return "Motor Overcurrent";
    case MOTOR_FAULT_OVERVOLTAGE:
        return "Supply Overvoltage";
    case MOTOR_FAULT_UNDERVOLTAGE:
        return "Supply Undervoltage";
    case MOTOR_FAULT_OVERTEMPERATURE:
        return "Driver Overtemperature";
    case MOTOR_FAULT_L6470_FLAG:
        return "L6470 Driver Fault";
    case MOTOR_FAULT_EMERGENCY_STOP:
        return "Emergency Stop";
    case MOTOR_FAULT_WATCHDOG_TIMEOUT:
        return "Watchdog Timeout";
    case MOTOR_FAULT_POSITION_ERROR:
        return "Position Error";
    }

    // Check system faults
    switch (fault_code) {
    case SYSTEM_FAULT_CLOCK_FAILURE:
        return "Clock Failure";
    case SYSTEM_FAULT_POWER_FAILURE:
        return "Power Failure";
    case SYSTEM_FAULT_STACK_OVERFLOW:
        return "Stack Overflow";
    case SYSTEM_FAULT_SAFETY_VIOLATION:
