 ...
 * @date 2025-01-29
 *
 * @note This file defines the central system state structure that serves as
 * the authoritative source for all system status information. All modules must
 * reference this single instance - never duplicate state information.
 *
...
typedef struct {
    ...
    MotorState_t
        current_state; // Current motor state (enum from motor_config.h)
